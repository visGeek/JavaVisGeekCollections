...
import jp.visgeek.utils.Action2;
import jp.visgeek.utils.ComparatorUtils;
...
public interface IEnumerable<T> extends Iterable<T> {
	...
	/***
	 * 整列済みのシーケンスから指定された要素を二分探索し、見つかった位置のインデックスを取得します。見つからなかった場合は検索値の次に大きい要素のインデックスのビットごとの補数です。
	 * @param item
	 * @return 見つかった位置のインデックス。見つからなかった場合は検索値の次に大きい要素のインデックスのビットごとの補数です。大きい要素が存在しない場合は要素数の補数です。
	 */
